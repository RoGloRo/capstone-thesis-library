...
const FloatingChat: React.FC = () => {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div className={cn(
          "fixed bottom-6 right-6 z-50 bg-white border border-gray-200 rounded-xl shadow-2xl transition-all duration-300 overflow-hidden",
          ...
          {!isMinimized && (
            <>
              {/* Messages Area */}
              <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gradient-to-b from-gray-50 to-white max-h-80 sm:max-h-96">
                {messages.length === 0 ? (
                    ...
                    <div className="space-y-1">
                      ...
                      {suggestedPrompts.map((prompt, index) => (
                        <button
                          ...
                          className="block w-full p-2 text-left bg-white border border-gray-200 rounded-lg hover:border-emerald-300 hover:bg-emerald-50 transition-colors text-xs"
                          disabled={isLoading}
                        >
                          {prompt}
                        </button>
                  ...
                  <>
                        ...
                        >
                          <div className="flex items-start gap-2">
                            {message.role === "assistant" && (
                              <Bot className="w-4 h-4 text-emerald-700 flex-shrink-0 mt-0.5" />
                            )}
                            {message.role === "user" && (
                    ...
                    {isLoading && (
                      <div className="flex justify-start">
                        <div className="bg-white p-3 rounded-2xl rounded-bl-md shadow-sm border border-gray-200 max-w-[80%]">
              ...
              <form onSubmit={sendMessage} className="border-t border-gray-200 p-3 bg-white">
                <div className="flex gap-2">
                  <textarea
                    ...
                    className="flex-1 p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent resize-none text-sm max-h-20"
                    disabled={isLoading}
                    maxLength={1000}
                    rows={1}
                    onKeyDown={(e) => {
                  ...
                  <button
                    ...
                    className="px-3 py-2 bg-gradient-to-r from-emerald-600 to-emerald-500 text-white rounded-lg hover:from-emerald-700 hover:to-emerald-600 focus:outline-none focus:ring-2 focus:ring-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 flex items-center justify-center"
                  >
                    {isLoading ? (
